...
test('Codex package provides default hook discovery and every referenced script', () => {
  const manifest = JSON.parse(fs.readFileSync(path.join(root, '.codex-plugin/plugin.json'), 'utf8'));
  assert.equal(manifest.name, 'research-skills');
  assert.equal(manifest.hooks, undefined);
  assert.ok(fs.existsSync(path.join(root, HOOKS_JSON)));
  assert.deepEqual(Object.keys(JSON.parse(fs.readFileSync(path.join(root, HOOKS_JSON), 'utf8')).hooks).sort(),
