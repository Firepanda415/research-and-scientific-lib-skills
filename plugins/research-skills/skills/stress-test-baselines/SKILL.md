...
description: "Assess, design, or run fair research comparisons and ablation studies when a claimed improvement depends on baseline quality, tuning budgets, component contributions, or robustness. Use for comparison audits, ablation study requests, and experiment design or execution; ordinary paper writing or reading alone does not trigger a new experiment campaign."
---
...
- **Execute:** perform and verify the authorized comparisons. Reuse valid existing
  results; start with a small representative case before expensive work.
...
Use fair bounded tuning and report the search space, effort, selected settings,
and meaningful sensitivity. Default settings are acceptable when justified for
the comparison; they are not automatically a strong or weak baseline.
...
Use seeds, regimes, datasets, tails, or limiting cases where they materially
affect the claim. Do not require every axis or component to be tested, and do not
run a full Cartesian sweep without a decision-relevant reason and budget.

For an ablation study, identify what is removed and which outcome can reveal its
contribution; check interactions when removing components separately could miss
them. Test-suite ablation measures lost defect detection against independently
confirmed defects and legal controls, including false positives and cost.
Production simplification instead checks preservation of scientific meaning,
public behavior, and resource requirements. Keep the relevant oracle independent
of the ablated component; all remaining tests passing alone does not establish
that removed code or evidence was unnecessary.
