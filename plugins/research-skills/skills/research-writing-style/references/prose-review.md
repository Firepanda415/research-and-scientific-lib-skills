...
For a substantial document, use an independent reviewer when available. Give that reviewer the complete draft, intended reader, relevant source facts, and task limits, without the writer's self-assessment or desired verdict. A final report or pull-request description that gives measured or computed results, makes scientific or compatibility claims, or reports the status of a multi-step task also gets an independent reviewer when available. Give that reviewer the sources the text describes, such as the diff, logs, and results, so that the reviewer can check those statements and the order of events and notice a missing compatibility statement. The reviewer must be free to reject the draft. A short passage outside these cases can receive a separate skeptical reread by the writer. The description of a routine change, such as a typo fix, needs only that reread, even when it mentions a count or says that behavior is unchanged. A user or project requirement to review every report or pull request independently takes precedence over these defaults. The absence of another agent never waives review, and using another agent never transfers responsibility for the final version.
...
For an extracted PDF or OCR text, or an excerpt without its document, first establish the source and role of the material, as [supplied-text review](supplied-text-review.md) describes.

Read the containing paragraph and the relevant preceding and following paragraphs before judging an excerpt. Follow necessary definitions, equations, figures, tables, and citations to recover its referents, comparison baseline, and scope. If the needed context is unavailable, distinguish a question to verify from a demonstrated omission. Do not expand a local review beyond the context needed to assess it.
