...
| CI, dependencies, and platform behavior | Do automation and tests exercise a distinct supported environment and fail actionably? | Workflow-to-policy comparison, base differential, platform-sensitive checks, and a repository-local check of preconditions only CI exposes, such as whether the workflow installs every module a test or probe imports, mapping import names to distributions where they differ | Workflow, dependency, packaging, precision, or environment changes, including a test or probe that needs a new optional dependency |
