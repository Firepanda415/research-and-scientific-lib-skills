...
Every generated or revised deliverable must pass an adversarial review, a stage separate from drafting, before delivery. Read and apply ${JSON.stringify(path.join(skill, 'references', 'prose-review.md'))} at that stage. That reference decides when an independent reviewer is used. Review-only requests start from the existing text and do not authorize a rewrite. Check especially for conversation/prompt leakage and user prohibitions turned into unnecessary negative prose. Review the final assembled version, fix verified defects within scope, and recheck changed passages before delivery.
