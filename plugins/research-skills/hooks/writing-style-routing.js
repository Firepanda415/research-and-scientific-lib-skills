...
Every generated or revised deliverable must pass an adversarial review, a stage separate from drafting, before delivery. Read and apply ${JSON.stringify(path.join(skill, 'references', 'prose-review.md'))} at that stage. That reference decides when an independent reviewer is used. Review-only requests start from the existing text and do not authorize a rewrite. Check especially for conversation/prompt leakage, and for user prohibitions or removed items turned into unnecessary negative prose. Review the final assembled version, fix verified defects within scope, and recheck changed passages before delivery.
...
SCOPE FROM THE REQUEST AND ITS AUDIENCE
Before planning a deliverable such as a document, code, or an analysis, infer what the requester intends, who will read or use the result, and what those readers or users come to it for. Keep to the scope the requester intended, and let that audience's need decide what to research, include, or build. When the work adopts a method or design from another work, such as a paper, a prototype, or other code, first establish from that source what its authors built it for and under which constraints. Judge it against this work's purpose on the dimensions that matter here, and adapt it where the purposes differ. Keeping, changing, or dropping an element that affects this work's results needs a reason tied to this work's purpose, not only the fact that the source did it. Stop gathering once the need is met, since filtering afterwards does not undo the cost of collecting. A claim's evidence standard sets how carefully it is checked, and the audience's need sets how many claims to seek. Briefs to other agents state the audience, the question or task, and when to stop.
